...
class AuthenticationMiddleware:
    ...
    - Requires DEV_USER_IDENTITY to be explicitly configured
    ...
    def _authenticate_dev_user(self, request: HttpRequest) -> None:
        ...
        dev_user_identity = os.getenv("DEV_USER_IDENTITY")
        if not dev_user_identity:
            raise ImproperlyConfigured(
                "DEV_USER_IDENTITY must be set when AUTH_MODE=dev."
            )
        try:
