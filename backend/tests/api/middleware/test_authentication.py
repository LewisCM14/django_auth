...
class TestAuthenticationMiddlewareDevMode:
    ...
    @override_settings(DEBUG=True)
    @pytest.mark.django_db
    def test_dev_mode_requires_explicit_identity(self) -> None:
        middleware = AuthenticationMiddleware(self.get_response)
        request = Mock()
        request.META = {}
        request.user = None
...
class TestAuthenticationMiddlewareIISMode:
    ...
    @pytest.mark.django_db
    def test_iis_mode_reads_windows_auth_token(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        middleware = AuthenticationMiddleware(self.get_response)
        monkeypatch.setattr(
            type(middleware.identity_resolver),
            "resolve",
            lambda _self, _token: "DOMAIN\\testuser",
        )
    ...
    @override_settings(DEBUG=False)
    @pytest.mark.django_db
    def test_iis_mode_reuses_existing_user(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        middleware = AuthenticationMiddleware(self.get_response)
        monkeypatch.setattr(
            type(middleware.identity_resolver),
            "resolve",
            lambda _self, _token: "DOMAIN\\existinguser",
        )
        ...
        assert User.objects.filter(
            username="DOMAIN\\existinguser", pk=original_pk
        ).exists()
...
class TestWindowsAuthIdentityResolver:
    ...
    def test_resolve_handles_os_error_during_impersonation(
        ...
        monkeypatch.setattr(
            ...
            lambda: (fake_api, BrokenSecurity()),
        )
